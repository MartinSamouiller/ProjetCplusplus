...
QImage CImageNdg::GetQImageNdg(void){
    /*QImage im = QImage(this->m_pucPixel, this->lireLargeur()+1, this->lireHauteur(), this->m_QImageBytePerLines , QImage::Format_Indexed8);
    QRgb value;
    for(int i = 0; i< 256; i++){
...
*/
    ...
    /// Probléme de padding dans QImage lorsque la largeur n'est pas multiple de 2
    /// Image déformée lorsqu'on la créée directement à partir du buffer image (uchar*)
...
void CImageNdg::sauvegarde() {
	...
	if (this->m_pucPixel) {
       //std::string nomFichier =  this->lireNom() + ".bmp"; // force sauvegarde dans répertoire Res (doit exister)
        std::string nomFichier = this->lireNom() + ".bmp"; // force sauvegarde dans répertoire Res (doit exister)
...
std::vector<unsigned long> CImageNdg::histogramme(bool enregistrementCSV, int pas) {
	...
	if (enregistrementCSV) {
        std::string fichier =  this->lireNom() + ".csv";
...
CImageNdg CImageNdg::transformation(const std::string& methode, int vMinOut, int vMaxOut) {
    ...
    std::vector<int> tmp_palette = this->lirePalette();
    out.choixPalette(tmp_palette); // conservation de la palette
	...
	if (methode.compare("erosion") == 0) {
		...
		if (eltStructurant.compare("V4") == 0) {
			...
			for (int j = 1; j < agrandie.lireLargeur() - 1; j++) {
                ...
                out(i - 1, j - 1) = qMin(minV4, (int)agrandie(i, j)); //cast a verifier
			}
		}
		else {
			if (eltStructurant.compare("V8") == 0) {
				...
				for (int j = 1; j < agrandie.lireLargeur() - 1; j++) {
                    ...
                    out(i - 1, j - 1) = qMin(minV8, (int)agrandie(i, j));
				}
			}
		}
	}
		...
		if (methode.compare("dilatation") == 0) {
			...
			if (eltStructurant.compare("V4") == 0) {
				...
				for (int j = 1; j < agrandie.lireLargeur() - 1; j++) {
                    ...
                    out(i - 1, j - 1) = qMax(maxV4, (int)agrandie(i, j)); //cast à verifier
				}
			}
			else {
				if (eltStructurant.compare("V8") == 0) {
					...
					for (int j = 1; j < agrandie.lireLargeur() - 1; j++) {
                        ...
                        out(i - 1, j - 1) = qMax(maxV8, (int)agrandie(i, j)); //cast à vérifier
					}
				}
			}
		}
	...
	if (methode.compare("moyennage") == 0) {
		...
		for (int j = 0; j < this->lireLargeur(); j++) {
			...
			for (int l = dl; l <= fl; l++) {
				...
				somme += 1;
				moy2 += (float)this->operator()(k, l)*(float)this->operator()(k, l);
			}
			...
			for (int j = 0; j < this->lireLargeur(); j++) {
				...
				for (int k = dk; k <= fk; k++)
					for (int l = dl; l <= fl; l++) {
						moy += (float)this->operator()(k, l);
						somme += 1;
						ecart += (float)this->operator()(k, l)*(float)this->operator()(k, l);
					}
				out(i, j) = (int)(sqrt((ecart / somme) - (moy / somme)*(moy / somme)));
			}
	}
	return out;
}
...
void CImageNdg::testEcriture(){
    qDebug() << "Bien le bonjour";
}
